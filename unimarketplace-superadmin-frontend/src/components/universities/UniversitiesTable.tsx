...
import {
  UniversityData,
  universitiesData,
} from '../../lib/types/universities.data';
import { AddUniversityModal } from './AddUniversityModal';

const { Search } = Input;

const UniversitiesTable: React.FC = () => {
  const [data, setData] = useState<UniversityData[]>(universitiesData);
  const [openAddModal, setOpenAddModal] = useState(false);

  const handleSubmit = (values: UniversityData) => {
        ...
        <div className='flex flex-col'>
          ...
          <span
            className={`font-semibold text-[13px] leading-[16px] font-space-grotesk ${
              status === 'Verified' ? 'text-[#16A34A]' : 'text-[#FFD037]'
            }`}
          >
            {status === 'Verified'
              ? en.table.verified
              : en.table.verificationPending}
          </span>
        </div>
      ),
    },
      ...
      >
        <div className='flex justify-between items-center mb-8'>
          ...
          <div className='flex items-center gap-[20px] mr-4'>
            <Plus
              className='w-5 h-5 text-[#041B4B] cursor-pointer hover:text-[#1B56CC]'
